...
class FantasyForgedSealedScraper(SealedScraper):
    """
    This is a bit different, we use playwright to scrape all pages of in stock sealed product

https://fantasyforged.ca/collections/mtg-sealed?filter.v.availability=1&filter.v.price.gte=&filter.v.price.lte=&sort_by=created-descending
    ...
    def scrape(self):
        ...
        try:
            ...
            try:
                ...
                cur.execute("SELECT * FROM sealed_prices WHERE website = 'fantasyforged' AND updated_at > NOW() - INTERVAL '8 hours'")
                rows = cur.fetchall()
            except:
                conn.rollback()
                rows = []
