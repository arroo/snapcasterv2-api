...
class FirstPlayerSealedScraper(SealedScraper):
    """
    ...
    def scrape(self):
        ...
        try:
            ...
            try:
                ...
                cur.execute("SELECT * FROM sealed_prices WHERE website = 'firstplayer' AND updated_at > NOW() - INTERVAL '8 hours'")
                rows = cur.fetchall()
            except:
                conn.rollback()
                rows = []
            ...
            # otherwise, scrape the site and update the database
                ...
                while nextPage is not None:
                    page = requests.get('https://www.FirstPlayer.ca'+ nextPage['href'])
                    sp = BeautifulSoup(page.text, 'html.parser')
                ...
                for product in allProducts:
                    link = 'https://www.FirstPlayer.ca' + product.select_one('div.image a')['href']
                    try:
                    ...
                    try:
                        stock = product.select_one('span.variant-qty').text.replace(" In Stock","").replace(' left','')
                    except:
                        stock = -1
            ...
            conn.close()
            print("FirstPlayerSealedScraper: Error on line {}".format(sys.exc_info()[-1].tb_lineno), type(e).__name__, e)