...
class TopDeckHeroSealedScraper(SealedScraper):
    """
...
https://www.topdeckhero.com/advanced_search?utf8=%E2%9C%93&search%5Bfuzzy_search%5D=&search%5Btags_name_eq%5D=&search%5Bsell_price_gte%5D=&search%5Bsell_price_lte%5D=&search%5Bbuy_price_gte%5D=&search%5Bbuy_price_lte%5D=&search%5Bin_stock%5D=0&search%5Bin_stock%5D=1&buylist_mode=0&search%5Bcategory_ids_with_descendants%5D%5B%5D=&search%5Bcategory_ids_with_descendants%5D%5B%5D=2153&search%5Bvariants_with_identifier%5D%5B19%5D%5B%5D=&search%5Bsort%5D=name&search%5Bdirection%5D=ascend&commit=Search&search%5Bcatalog_group_id_eq%5D=
   """

    def __init__(self, setName):
        SealedScraper.__init__(self, setName)
        self.website = 'topdeckhero'
        self.url = 'https://www.topdeckhero.com/advanced_search?utf8=%E2%9C%93&search%5Bfuzzy_search%5D=&search%5Btags_name_eq%5D=&search%5Bsell_price_gte%5D=&search%5Bsell_price_lte%5D=&search%5Bbuy_price_gte%5D=&search%5Bbuy_price_lte%5D=&search%5Bin_stock%5D=0&search%5Bin_stock%5D=1&buylist_mode=0&search%5Bcategory_ids_with_descendants%5D%5B%5D=&search%5Bcategory_ids_with_descendants%5D%5B%5D=2153&search%5Bvariants_with_identifier%5D%5B19%5D%5B%5D=&search%5Bsort%5D=name&search%5Bdirection%5D=ascend&commit=Search&search%5Bcatalog_group_id_eq%5D='
    ...
    def scrape(self):
        ...
        try:
            ...
            try:
                ...
                cur.execute("SELECT * FROM sealed_prices WHERE website = 'topdeckhero' AND updated_at > NOW() - INTERVAL '8 hours'")
                rows = cur.fetchall()
            except:
                conn.rollback()
                rows = []
            ...
            # otherwise, scrape the site and update the database
                ...
                # should be ul.'product detailed' li.product div.inner
                # products = 
                products = sp.select('div.products-container li.product div.inner')
                ...
                while nextPage is not None:
                    page = requests.get('https://topdeckhero.com'+ nextPage['href'])
                    sp = BeautifulSoup(page.text, 'html.parser')
                ...
                for product in allProducts:
                    link = 'https://www.topdeckhero.com' + product.select_one('div.image a')['href']
                    name = product.select_one('h4.name').text.strip()
                    imageUrl = product.select_one('img')['src']
                    # CAD$ 1,649.99
                    
                    price = product.select_one('div.product-price-qty span.price')
                    if not price:
                        # print a dbugging message with the name, link, and image url
                        print(f'No price found for {name} {link} {imageUrl}')
                        print(price)
                        continue
                    price = price.text.replace("CAD$ ","").replace(",","").strip()
                    # price = product.select_one('div.product-price-qty span.price')
                    # price = product.select_one('div.variant-row').select_one('form.add-to-cart-form')['data-price'].replace("CAD$ ","")
                    try:
                        stock = product.select_one('div.variant-row em').text.replace(" In Stock","").replace(' left','')
                    except:
                        stock = -1
