...
class OrchardCitySealedScraper(SealedScraper):
    """
    ...
    def scrape(self):
        ...
        try:
            ...
            try:
                ...
                cur.execute("SELECT * FROM sealed_prices WHERE website = 'orchardcity' AND updated_at > NOW() - INTERVAL '8 hours'")
                rows = cur.fetchall()
            except:
                conn.rollback()
                rows = []
            ...
            # otherwise, scrape the site and update the database
                ...
                while nextPage is not None:
                    page = requests.get('https://www.orchardcitygames.com'+ nextPage['href'])
                    sp = BeautifulSoup(page.text, 'html.parser')
                ...
                for product in allProducts:
                    link = 'https://www.orchardcitygames.com' + product.select_one('div.image a')['href']
                    try:
            ...
            conn.close()
            print("OrchardCitySealedScraper: Error on line {}".format(sys.exc_info()[-1].tb_lineno), type(e).__name__, e)