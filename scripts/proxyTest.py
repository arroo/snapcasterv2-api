...
This is a script to test proxies from the proxies.txt file.

Create a /scripts/proxies.txt file with proxies in the format:
ip_address:port:username:password

This script will test each proxy and print the ones that work by
running an example request to the Mythic Store api.
...
def checkProxy(proxy):
    # This is the request we send to Mythic Store games, but it's basically identical for all the shopify sites.
    body = {
    ...
    try:
        r = requests.post(
            ...
            proxies=proxies,
            timeout=5,
        )  # this doesn't work and returns 404s
