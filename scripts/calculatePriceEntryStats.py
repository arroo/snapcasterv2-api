"""
calculatePriceEntryStats.py

Retroactively calculate the max, min, and avg prices for each price_entry in the database
that doesn't have those fields.

This was used before I started calculating the max, min, and avg prices upon price entry
creation.
...
for entry in price_entries:
    ...
    if len(price_list) == 0:
        ...
        max_price = max(
            [float(str(price["price"]).replace(",", "")) for price in price_list]
        )
        min_price = min(
            [float(str(price["price"]).replace(",", "")) for price in price_list]
        )
        ...
        if len(foil_price_list) != 0:
            foil_max_price = max(
                [
                    float(str(price["price"]).replace(",", ""))
                    for price in foil_price_list
                ]
            )
            foil_min_price = min(
                [
    ...
    db.price_entry.update_one({"_id": entry["_id"]}, {"$set": updated_entry})
