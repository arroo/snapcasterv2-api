...
Template script to clean outlier data from the database for a single oracle id.
...
def remove_incorrect_data(oracle_id):
    ...
    for price_entry in price_entries:
        ...
        # Update the avg, min, max, foil_avg, foil_min, foil_max
        price_entry["avg"], price_entry["min"], price_entry["max"] = update_price_stats(
            normal_prices
        )
        (
            price_entry["foil_avg"],
            price_entry["foil_min"],
            price_entry["foil_max"],
        ) = update_price_stats(foil_prices)

        # Save the updated price_entry
        priceEntryCollection.update_one(
            {"_id": price_entry["_id"]}, {"$set": price_entry}
        )
